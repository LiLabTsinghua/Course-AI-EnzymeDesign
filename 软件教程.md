...
我们建议选择版本[1.96.4](https://code.visualstudio.com/updates/v1_96)，并且在设置中关闭自动更新，以使得SSH远程连接稳定。  
请根据你的操作系统选择合适的版本。以下教程以Windows 10为例。

### ⚙️ 1.1 VSCode for Windows 10安装步骤
...
4. **安装Python和Jupyter插件**：
   - 在拓展界面，选择在SSH:xx.xxx.xx.xxx中，搜索并安装“Python”和“Jupyter”插件。
...
1. **运行实例**：
   - 确保你已经在**服务器**安装了Python和Jupyter插件，随后打开课件中的jupyter notebook（后缀名为ipynb的文件）
   - 在软件的左上角菜单栏-查看-终端可以打开命令行，环境的安装需要在终端中执行
